...
type Graphy struct {
	mutators map[string]GraphFunction
}
...
func (g *Graphy) RegisterMutatorWithParamNames(ctx context.Context, name string, mutatorFunc any, names ...string) {
	gf := NewGraphFunction(name, mutatorFunc, names...)
	if g.mutators == nil {
		g.mutators = map[string]GraphFunction{}
	}
	g.mutators[name] = gf
}

func (g *Graphy) ProcessRequest(ctx context.Context, request string) (string, error) {
